#!/usr/bin/env python3
"""Train GazeQueryNet (2-query ViT attention) on gaze_spiral_ego labels."""
...
from gaze_heatmap.train import evaluate, masked_mse, split_episodes
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/mnt/hdd/dexjoco/outputs/gaze_query_train_vit"),
    )
    ...
    model = GazeQueryNet(image_size=args.image_size, num_keypoints=2).to(device)
    opt = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.LambdaLR(opt, lambda step: min(step / 100, 1.0))

    args.out_dir.mkdir(parents=True, exist_ok=True)
    meta = {
        "model": "GazeQueryNet-vit_small_patch16_224",
