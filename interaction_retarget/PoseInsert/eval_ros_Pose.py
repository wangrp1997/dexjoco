...
parser.add_argument('--ckpt', action='store', type=str, help='checkpoint path', required=False,
                    default='/home/agilex/sunhan/Pose_EDP_insert/logs/e_cnn2_sample/policy_last.ckpt')
...
class ImageProcessor:
    def __init__(self):
        ...
        rospy.Subscriber("/source/pose", PoseStamped, self.source_pose_callback) ## source target
        rospy.Subscriber("/right/pose_info", PoseStamped, self.arm_pose_callback) ## source target
    ...
    def process_images(self):
        ...
        policy = PoseDP(
            num_action=20,
            input_dim=9,
            obs_feature_dim=128,  ##args.obs_feature_dim
            action_dim=9,
            hidden_dim=512,  ##args.hidden_dim
            prediction="sample",  ## epsilon
        ).to(device)
        ...
        while not rospy.is_shutdown():
            ...
            with torch.inference_mode():
                ...
                pose = self.pose_to_matrix(pose)[:3, :4]
                ...
                if args.vis:
                    ...
                    for i in range(20):
                        ...
                        color = create_coor(color, source_pose[:3, :4], intrinsic_matrix,corners_3D_size=0.02)
                        color = create_coor(color, target_pose[:3, :4], intrinsic_matrix,corners_3D_size=0.02)
                        color = create_coor(color, T_source_in_cam[:3, :4], intrinsic_matrix,corners_3D_size=0.05 )
