...
for idx in range(7):
    source_pose = np.load(data_path + '/{}/source_pose.npy'.format(idx))
    target_pose = np.load(data_path + '/{}/target_pose.npy'.format(idx))


    source_action = source_pose[len(source_pose)-1]
    target_action = target_pose[len(source_pose)-1]


    source = pose_to_matrix(source_action)
    target = pose_to_matrix(target_action)
    ...
    source_in_target_trajectories_all.append(source_in_target)



print('**********************************************************************************')

source_in_target_trajectories_all = np.array(source_in_target_trajectories_all)

print("x   max: {}, min: {}".format(np.max(source_in_target_trajectories_all[:,0,3]),np.min(source_in_target_trajectories_all[:,0,3]) ))
print("y   max: {}, min: {}".format(np.max(source_in_target_trajectories_all[:,1,3]),np.min(source_in_target_trajectories_all[:,1,3]) ))
print("z   max: {}, min: {}".format(np.max(source_in_target_trajectories_all[:,2,3]),np.min(source_in_target_trajectories_all[:,2,3]) ))

print('**********************************************************************************')

print("x   error: {}".format(np.max(source_in_target_trajectories_all[:,0,3])-np.min(source_in_target_trajectories_all[:,0,3]) ))
print("y   error: {}".format(np.max(source_in_target_trajectories_all[:,1,3])-np.min(source_in_target_trajectories_all[:,1,3]) ))
print("z   error: {}".format(np.max(source_in_target_trajectories_all[:,2,3])-np.min(source_in_target_trajectories_all[:,2,3]) ))
