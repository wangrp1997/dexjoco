#!/usr/bin/env bash
# Right22 Diffusion 50k handoff eval (10 ep): tray weld left + freeze left mocap.
set -euo pipefail

REPO=/home/wangrenpeng/dexjoco
OUT=/mnt/hdd/dexjoco/outputs/insert_bc_handoff_eval_right22_diffusion_weld10
...
CUDA_VISIBLE_DEVICES=1 $SERVE --host 127.0.0.1 --port 8098 > "$OUT/diff_server.log" 2>&1 &
SPID=\$!
sleep 12
...
tmux kill-session -t eval_right22_diff_weld10 2>/dev/null || true
tmux new-session -d -s eval_right22_diff_weld10 "$OUT/run_diff.sh"
echo "Started tmux eval_right22_diff_weld10 -> $OUT"
