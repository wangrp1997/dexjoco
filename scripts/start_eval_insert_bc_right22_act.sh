#!/usr/bin/env bash
# Right22 ACT 50k handoff eval: freeze left + tray snap + stronger left grip.
set -euo pipefail

REPO=/home/wangrenpeng/dexjoco
OUT=/mnt/hdd/dexjoco/outputs/insert_bc_handoff_eval_right22_act
...
CUDA_VISIBLE_DEVICES=2 $SERVE --host 127.0.0.1 --port 8095 > "$OUT/act_server.log" 2>&1 &
SPID=\$!
sleep 12
...
tmux kill-session -t eval_right22_act 2>/dev/null || true
tmux new-session -d -s eval_right22_act "$OUT/run_act.sh"
echo "Started tmux eval_right22_act -> $OUT"
