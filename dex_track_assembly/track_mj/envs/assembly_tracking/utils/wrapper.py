...
class ModifiedEpisodeWrapper(Wrapper):
    ...
    def step(self, state: State, action: jax.Array, trajectory_data) -> State:
        ...
        # Aggregate state metrics into episode metrics (clear accumulators on new episode)
        prev_done = state.info["episode_done"]
        keep = 1.0 - prev_done
        ep = state.info["episode_metrics"]
        ep["sum_reward"] = ep["sum_reward"] * keep + jp.sum(rewards, axis=0)
        ep["length"] = ep["length"] * keep + self.action_repeat
        safe_len = jp.maximum(ep["length"], 1.0)
        ep["average_sum_reward"] = ep["sum_reward"] / safe_len

        for metric_name in state.metrics.keys():
            if metric_name == "reward":
                continue
            ep[metric_name] = ep[metric_name] * keep + state.metrics[metric_name]
            ep["average_" + metric_name] = ep[metric_name] / safe_len
