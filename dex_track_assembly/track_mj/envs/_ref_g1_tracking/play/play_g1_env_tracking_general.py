...
@tmj.registry.register("G1TrackingGeneral", "tracking_play_env_class")
class PlayG1TrackingGeneralEnv:
    ...
    def __init__(
        ...
        if self.use_renderer:
            ...
            self.writer = None
            os.makedirs(f"storage/videos/track/{self.exp_name}/{self.terrain_type}", exist_ok=True)
    ...
    def reset(self):
        ...
        if self.use_renderer:
            os.makedirs(f"storage/videos/track/{self.exp_name}/{self.terrain_type}/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][0]}", exist_ok=True)

            self.writer = imageio.get_writer(
                f"storage/videos/track/{self.exp_name}/{self.terrain_type}/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][0]}/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][1]}.mp4",
                fps=50,
            )
