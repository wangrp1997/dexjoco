# Source: refs/ForceVLA/src/openpi/models/pi0_force.py (ForceVLA), adapted for DexJoCo dual-arm.
import dataclasses
import logging
import einops
...
class Pi0ForceDexJoCoConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0ForceDexJoCo":
        return Pi0ForceDexJoCo(self, rngs=nnx.Rngs(rng))
...
class Pi0ForceDexJoCo(_model.BaseModel):
    def __init__(self, config: Pi0ForceDexJoCoConfig, rngs: nnx.Rngs):
        super().__init__(config.action_dim, config.action_horizon, config.max_token_len)
        self._proprio_dim = config.proprio_dim
        self._force_dim = config.force_dim
        self._loss_weight_tuple = None
        ...
        self.state_proj = nnx.Linear(config.proprio_dim, action_expert_config.width, rngs=rngs)
    ...
    def embed_suffix(
        ...
        ar_mask = []
        tokens = []
        # obs.state layout: [:proprio_dim] proprio, [proprio_dim:proprio_dim+force_dim] force/tactile.
        proprio = obs.state[:, : self._proprio_dim]
        state_token = self.state_proj(proprio)[:, None, :]
