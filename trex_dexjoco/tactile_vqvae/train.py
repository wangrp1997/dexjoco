...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--granularity", type=str, default="finger",
                   choices=["hand", "finger"],
                   help="hand: 1 code per (hand, window).  finger: n_fingers codes.")
    p.add_argument("--n_fingers", type=int, default=4,
                   help="Fingers per hand (DexJoCo Allegro=4; upstream Sharpa=5).")
    p.add_argument("--per_finger_dim", type=int, default=3,
                   help="Force dims per finger (DexJoCo xyz=3; upstream F6=6).")
...
def _build_config(args) -> TactileVQVAEConfig:
    n_fingers = int(getattr(args, "n_fingers", 4))
    per_finger_dim = int(getattr(args, "per_finger_dim", 3))
    return TactileVQVAEConfig(
        window=args.window,
        in_channels=n_fingers * per_finger_dim,
