...
from utils.lerobot_common import (
    F6_DIM, F6_PER_FINGER, N_FINGERS,
)
...
class SftDataset(Dataset):
    ...
    def _build_f6_history_index(self):
        """Group per-sample tactile_f6 by episode (sorted by frame).

        Prefer explicit `episode_id` + `frame_index` (DexJoCo JSON / adapter).
        Fall back to deform-path regex (upstream) when those fields are absent.
        """
        ...
        for i in range(n):
            s = self.hf_dataset[i]
            ep_dir = s.get("episode_id")
            frame = s.get("frame_index")
            if ep_dir is None or frame is None:
                deforms = s.get("tactile_image_deform", []) or []
                if not deforms:
                    # Fallback: parse from slow image path
                    slow = (s.get("input_image_slow") or [None])[0]
                    if not slow:
                        continue
                    m = frame_re.search(slow) or re.search(
                        r"(episode[_\d]+).*?(\d+)", str(slow))
                    if not m:
                        continue
            ...
            by_ep.setdefault(str(ep_dir), []).append((frame, f6))
        ...
        self.accelerator.print(
            ...
            f"window={self.vqvae_window}, shape=[{N_FINGERS},{F6_PER_FINGER}]")

    def _f6_history_for_sample(self, sample) -> np.ndarray:
        """Return raw [window, N_FINGERS, F6_PER_FINGER] F6 history."""
        W = self.vqvae_window
        frame_re = re.compile(r"(.+/episode_\d+)/image(\d+)_")
        cur = np.asarray(sample["tactile_f6"], dtype=np.float32).reshape(
            1, N_FINGERS, F6_PER_FINGER)
        ep_dir = sample.get("episode_id")
        frame = sample.get("frame_index")
        if ep_dir is None or frame is None:
            deforms = sample.get("tactile_image_deform", []) or []
            m = frame_re.search(deforms[0]) if deforms else None
            if m is None:
                slow = (sample.get("input_image_slow") or [None])[0]
                m = frame_re.search(slow) if slow else None
            if m is None:
                return np.repeat(cur, W, axis=0)
            ep_dir, frame = m.group(1), int(m.group(2))
        else:
            ep_dir, frame = str(ep_dir), int(frame)
    ...
    def collate_fn(self, batch: List[Dict]) -> Dict:
        ...
        if cfg.use_tactile_vec:
            ...
            norm_tacf6 = torch.tensor(norm_tacf6.reshape(B, -1, F6_PER_FINGER), dtype=torch.bfloat16)
        ...
        if getattr(cfg, "use_tactile_vqvae", 0):
            ...
            if have_codes:
                ...
                hist = np.stack(
                    [self._f6_history_for_sample(x) for x in batch], axis=0)  # [B, W, NF, D]
        ...
        return {
            ...
            "tactile_f6_history": tactile_f6_history_tensor,  # [B, W, NF, D] raw or None
            "time_r": time_r,
            "eps_r": eps_r,
            "state_raw": state_raw,
...
class TrainingMetrics:
    def __init__(self, device, world_size: int = 1):
        self.n_step       = 0
...
def train(args):
    ...
    if accelerator.state.deepspeed_plugin is None:
        ...
        accelerator.print("Cast model to bfloat16 (non-DeepSpeed path).")
    ...
    elif getattr(args, "data_format", "json") == "dexjoco":
        from adapters.dexjoco_dataset import DexJoCoSftDataset
        dataset = DexJoCoSftDataset(args, processor, accelerator)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_format", type=str, default="json",
                        choices=["json", "lerobot", "dexjoco"])
    parser.add_argument("--lerobot_root", type=str, default="",
                        help="LeRobot dataset dir (lerobot / dexjoco formats).")
    parser.add_argument("--lerobot_repo_id", type=str, default="",
                        help="Optional repo_id; defaults to basename(lerobot_root).")
    parser.add_argument("--force_labels_path", type=str, default="",
                        help="DexJoCo force_labels/forces.parquet (data_format=dexjoco).")
    parser.add_argument("--norm_stats_path", type=str, default="",
                        help="Path to trex_norm_stats.json for dexjoco format.")
    parser.add_argument("--language_instruction", type=str,
                        default="Assemble the peg into the tray.",
                        help="Task language for DexJoCo post-train.")
    ...
    elif args.data_format == "dexjoco":
        if not args.lerobot_root:
            parser.error("--data_format dexjoco requires --lerobot_root")
        if not args.force_labels_path:
            parser.error("--data_format dexjoco requires --force_labels_path")
        if not args.norm_stats_path:
            parser.error("--data_format dexjoco requires --norm_stats_path "
                         "(run adapters/compute_norm_stats.py first)")
