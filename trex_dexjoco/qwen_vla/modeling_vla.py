...
class Qwen3VLVLAModel(nn.Module):
    ...
    def __init__(
        ...
        if self.use_tactile_vqvae:
            ...
            # F6 min-max stats: flat dim = 2 hands × n_fingers × per_finger_dim
            # (DexJoCo: 2×4×3=24; upstream Sharpa: 2×5×6=60).
            _nf = int(vqvae_config.get("n_fingers", 5))
            _pd = int(vqvae_config.get("per_finger_dim", 6))
            _flat = 2 * _nf * _pd
            self.register_buffer("tacf6_vqvae_min",
                                 torch.zeros(_flat, dtype=torch.float32), persistent=True)
            self.register_buffer("tacf6_vqvae_max",
                                 torch.ones(_flat, dtype=torch.float32), persistent=True)
            self.register_buffer("tacf6_vqvae_mask",
                                 torch.ones(_flat, dtype=torch.bool), persistent=True)
    ...
    def from_pretrained_qwen3vl(
        ...
        if vqvae_config is not None and tacf6_dim == 6:
            # Prefer per_finger_dim from DexJoCo / retrained VQ-VAE config.
            tacf6_dim = int(vqvae_config.get("per_finger_dim", tacf6_dim))
    ...
    def encode_tactile_f6_history(self, f6_history_raw: torch.Tensor) -> torch.Tensor:
        ...
        f6_history_raw : [B, T, NF, D] raw (DexJoCo: NF=8,D=3; upstream: 10,6).
                         Normalized internally with the embedded VQ-VAE stats.

        Returns
        -------
        codes : [B, n_codes] int64 for `tactile_code_embedder`:
                  hand   granularity → [B, 2]
                  finger granularity → [B, 2*n_fingers]
        """
        ...
        if NF % n_per != 0:
            raise ValueError(
                f"tactile history NF={NF} not divisible by vqvae n_fingers={n_per}")
        n_hands = NF // n_per
        per_hand_codes = []
        for h in range(n_hands):
            wh = normed[:, :, h * n_per:(h + 1) * n_per, :].to(w_dtype)
            idx = vq.encode(wh)
            if idx.dim() == 1:
                idx = idx.unsqueeze(1)
            per_hand_codes.append(idx.long())
        return torch.cat(per_hand_codes, dim=1)
